...
module.exports = app => {
    class cube extends app.Service {
            ...
            return new Promise((resolve, reject) => {
                co(ctx.service.xueqiu.request("https://xueqiu.com/P/" + id, cookie)).then(function (sp) {
                    ...
                    $('.weight-list .stock').each(function (i, item) {
                        ...
                        co(ctx.service.cube.addCubePosition({id, date, uid, stock_name, stock_code, stock_weight}));
                        //cube_postions
        ...
        * addCubePosition(data){
            let ctx = this.ctx;
            let conditions = {id: data.id, uid:data.uid, date: data.date, stock_code:data.stock_code};
